...
코로나로 인해 회사가 커지면서 운영중인 서버의 사용량이 기하급수적으로 늘어났었다.

이에따라 2020년도엔 RabbitMQ, AWS DynamoDB 도입 등등 트래픽을 분산시키기 위한 작업들이 많았던거 같다. 특히 Pinpoint 모니터링 툴을 통해 병목 구간을 찾을 수 있었고 몇몇의 로직에서 N+1 쿼리가 발생하고 있다는 것을 알 수 있었다.

병목 구간은 의외였다. 주로 사용자 옵션과 같이 일대일 관계에서 발생하고 있기 때문이었다. 일반적으로 개발자들이 N+1 문제를 *ToMany 관계에서 주로 일어난다고 생각하는 것 같다. 따라서 *ToMany 관계에선 주의 깊게 로직을 구현하지만 비교적 OneToOne 관계에서도 N+1 문제가 발생한다는 점을 안일하게 생각했던 건 아니었는지 싶다.

정리하자면 LAZY 패치는 메모리 성능과 특정 도메인을 조회 시 불필요한 컬럼이 포함하지 않기 때문에 비즈니스를 파악하기 수월하다는 장점이 존재하지만, 자칫 잘못 사용하는 경우   LazyInitializationException 와 N+1 문제를 직면하여 오히려 성능상의 이슈가 발생할 수 있다. JPQL의 N+1 문제와 해결 방법에 대해 인지하고 각각 상황에 맞게 비즈니스 로직을 검증할 필요가 있다.
