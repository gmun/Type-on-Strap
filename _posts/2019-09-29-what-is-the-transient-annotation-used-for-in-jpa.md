...
4. **<U>QueryDSL 또한 JPA의 엔티티 접근 방식을 기준으로 QDomain 클래스를 생성한다.</U>** 
    - 따라서 @Transient가 선언된 필드나 메서드는 생성과정에서 제외된다.
    ...
    - com.moong.api
        - member
            ...
            - service
                ...
                - MemberManagerImpl.java  <-- [3] @Repository 인터페이스 구현체
...
또한, 단순한 비즈니스 로직이라면 @Entity 클래스에 구성하는 게 옳지 않을까 생각합니다. 하지만 아직은 경험이 부족하여, 어떻게 @Entity 객체를 어디까지 도메인 객체로 허용해야 하는지는 감이 잡히질 않습니다. 이에 대한 답을 미래에 저에게 맡기며 글을 마치려 합니다.
