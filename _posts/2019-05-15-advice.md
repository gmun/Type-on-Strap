...
물론 @Advice라는 애노테이션은 없지만, 이해를 돕기위해 사용했습니다. 다음 코드처럼 @AspectJ 5 라이브러리에서 제공하는 어드바이스 애노테이션를 표기하고 포인트 컷이나 직접적으로 조인 포인트를 명시하여 구현해주셔야 합니다. 포인트 컷에 대한 부분은 이전에 작성한 ["포인트 컷의 다양한 표현식"](https://gmun.github.io/spring/aop/2019/05/06/pointcut.html) 포스팅을 참고해주시기 바랍니다.
...
4. @Around(주위)
5. @After(이후)
6. @AfterReturning(완료) || @AfterThrowing(예외)
