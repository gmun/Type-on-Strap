...
다음과 같은 의미를 빗대어 프로그램 측면에서 해석하자면 반복되는 코드가 이에 해당한다고 볼 수 있다. 이러한 코드를 횡단 관심사(Crosscutting Concerns)라 표현한다. AOP에선 이러한 관심사(Concerns)를 기준으로 크게 핵심 관심사와 횡단 관심사로 분류하고 있다.
...
#### OOP의 극단적인 추상화의 한계
 ...
 따라서 횡단 관심사를 분리하기 위해 핵심 관심사와 횡단 관심사를 따로 객체를 생성하여 만든다. 이때 유연한 확장을 하기 위해 원래 클래스는 추상화로 구현하고 추상화 클래스에 횡단 관심사가 적용해 이를 구현 객체를 사용한다.

여기까진 완벽해 보인다. 하지만 다음과 같은 상황이 닥친다면 어떻게 될까?
