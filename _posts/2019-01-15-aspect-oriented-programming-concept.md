...
따라서 횡단 관심사를 `추상화`와 `템플릿 메소드 패턴`과 같은 디자인 패턴을 통해 기존 클래스에서 횡단 관심사와 핵심 관심사를 분리하고 각각의 독립적인 모듈로 분리하여 관리할 수 있다.
...
- UserService : 핵심 로직 추상화
- UserServiceImple : 핵심 로직
- UserServiceTX : 횡단 로직

설계적 측면으로 하나의 클래스에서 2개의 클래스가 분리되었다.


기존 UserService는 인터페이스로 정의하고 횡단 관심사


기존 UserService 클래스는 추상화 클래스로 정의하고 해당 추상화 메소드는 기존의 핵심 메소드를 정의한다.
